...
-- CREATE OR REPLACE DYNAMIC TABLE DOWNLOAD 
--     (
--         AGG_PERIOD VARCHAR(16777216) COMMENT 'The dimension of the aggregate (e.g., YEARLY, MONTHLY, DAILY).',
--         AGG_YEAR NUMBER(38,0) COMMENT 'Primary key (composite). Year of the aggregation period.',
--         AGG_QUARTER NUMBER(38,0) COMMENT 'Primary key (composite). Quarter of the aggregation period.',
--         AGG_MONTH NUMBER(38,0) COMMENT 'Primary key (composite). Month of the aggregation period.',
--         AGG_DAY NUMBER(38,0) COMMENT 'Primary key (composite). Day of the aggregation period.',
...
--         USERS INT COMMENT 'The number of unique users that have generated a pre-signed URL for this entity during the aggregation period. This is an approximation of a download.'
--     )
--     TARGET_LAG = '1 day'
...
--         FROM {{database_name}}.SYNAPSE_RAW.FILEUPLOAD --noqa: JJ01,PRS,TMP
--         -- exclude today’s partial data to avoid confusion in the final table
--         WHERE RECORD_DATE < (SELECT MAX(RECORD_DATE) FROM {{database_name}}.SYNAPSE_RAW.FILEUPLOAD) --noqa: JJ01,PRS,TMP
