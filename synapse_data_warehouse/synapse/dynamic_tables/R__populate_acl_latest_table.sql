...
-- 2. Create the first temporary table which unpacks the resource_access column
--    and extracts the access type and principal ID as new columns
CREATE TEMPORARY TABLE acl_expanded_temp AS
    SELECT
        owner_id,
        change_timestamp,
        COALESCE(
...
-- 3. Create the second temporary table which deduplicates the data from the table
--    created in the previous step based on owner_id, access_type, and principal_id
CREATE TEMPORARY TABLE dedup_acl_expanded_temp AS
    SELECT
        owner_id, 
        change_timestamp, 
        access_type, 
...
-- 4. Create the dynamic table ACL_LATEST from the deduplicated temporary table
CREATE OR REPLACE DYNAMIC TABLE ACL_LATEST
    ...
    SELECT
        ...
        principal_id
    FROM
        dedup_acl_expanded_temp;

-- 5. Cleanup: Drop the temporary tables
DROP TABLE IF EXISTS acl_expanded_temp;
DROP TABLE IF EXISTS dedup_acl_expanded_temp;
