...
AS
    COPY INTO asynch_job_status FROM (
        SELECT
            ...
            $1:CANCELING::BOOLEAN                         AS canceling,
...
AS
    COPY INTO discussion_reply FROM (
        SELECT
            ...
            $1:IS_EDITED::BOOLEAN                         AS is_edited,
...
AS
    COPY INTO discussion_search_index FROM (
        SELECT
            ...
            $1:THREAD_ID::BIGINT                          AS thread_id,
            $1:THREAD_DELETED::BOOLEAN                    AS thread_deleted,
            $1:REPLY_ID::BIGINT                           AS reply_id,
            $1:REPLY_DELETED::BOOLEAN                     AS reply_deleted,
...
AS
    COPY INTO discussion_thread FROM (
        SELECT
            ...
            $1:IS_EDITED::BOOLEAN                         AS is_edited,
            $1:IS_DELETED::BOOLEAN                        AS is_deleted,
            $1:IS_PINNED::BOOLEAN                         AS is_pinned,
            SPLIT_PART(METADATA$FILENAME, '/', 3)::BIGINT AS stack,
            SPLIT_PART(METADATA$FILENAME, '/', 4)::DATE   AS snapshot_date,
            METADATA$FILENAME                             AS filename
...
AS
    COPY INTO files FROM (
        SELECT
            ...
            $1:IS_PREVIEW::BOOLEAN                        AS is_preview,
...
AS
    COPY INTO grid_replica FROM (
        SELECT
            ...
            $1:CREATED_ON::TIMESTAMP_NTZ(9)               AS created_on,
            $1:SESSION_ID::VARCHAR                        AS session_id,
            $1:IS_AGENT::BOOLEAN                          AS is_agent,
            SPLIT_PART(METADATA$FILENAME, '/', 3)::BIGINT AS stack,
            SPLIT_PART(METADATA$FILENAME, '/', 4)::DATE   AS snapshot_date,
            METADATA$FILENAME                             AS filename
...
AS
    COPY INTO json_schema_object_binding FROM (
        SELECT
            ...
            $1:CREATED_BY::BIGINT                         AS created_by,
            $1:CREATED_ON::TIMESTAMP_NTZ(9)               AS created_on,
            $1:ENABLE_DERIVED::BOOLEAN                    AS enable_derived,
            SPLIT_PART(METADATA$FILENAME, '/', 3)::BIGINT AS stack,
            SPLIT_PART(METADATA$FILENAME, '/', 4)::DATE   AS snapshot_date,
            METADATA$FILENAME                             AS filename
...
AS
    COPY INTO message_to_user FROM (
        SELECT
            ...
            $1:WITH_UNSUBSCRIBE_LINK::BOOLEAN             AS with_unsubscribe_link,
            $1:WITH_PROFILE_SETTING_LINK::BOOLEAN         AS with_profile_setting_link,
            $1:IS_NOTIFICATION_MESSAGE::BOOLEAN           AS is_notification_message,
            $1:OVERRIDE_NOTIFICATION_SETTINGS::BOOLEAN    AS override_notification_settings,
            $1:PRIMARY_RECIPIENTS::BINARY                 AS primary_recipients,
            $1:CC::BINARY                                 AS cc,
            $1:BCC::BINARY                                AS bcc,
            $1:SENT::BOOLEAN                              AS sent,
            SPLIT_PART(METADATA$FILENAME, '/', 3)::BIGINT AS stack,
            SPLIT_PART(METADATA$FILENAME, '/', 4)::DATE   AS snapshot_date,
            METADATA$FILENAME                             AS filename
...
AS
    COPY INTO oauth_client FROM (
        SELECT
            ...
            $1:IS_VERIFIED::BOOLEAN                       AS is_verified,
            $1:JSON::VARCHAR                              AS json,
            $1:CREATED_BY::BIGINT                         AS created_by,
            $1:CREATED_ON::BIGINT                         AS created_on,
            $1:MODIFIED_ON::BIGINT                        AS modified_on,
