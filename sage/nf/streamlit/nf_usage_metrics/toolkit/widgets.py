...
def plot_stacked_bar_chart(df: pd.DataFrame, width: int = 1400) -> go.Figure:
    ...
    status_counts = df.groupby([project_status_col, data_status_col]).size().unstack(fill_value=0)
...
def plot_download_sizes(df, project_sizes_df, width=1600):
    ...
    df["TOTAL_CONTENT_SIZE"] = df["PROJECT_ID"].map(
...
def plot_download_sizes_lollipop(df, color=PORTAL_BLUE, width=1600):
    ...
    y0 = y_min / 10 if y_min > 0 else df[df["TOTAL_DOWNLOADS"] > 0]["TOTAL_DOWNLOADS"].min() / 10
...
def plot_resource_downloads(df, resource_column, color="teal", title=None, count_column="DOWNLOAD_COUNT", width=1600, height=600):
...
def merge_size_and_downloads(df_size, df_downloads):
    ...
    merged_df['TRUNCATED_NAME'] = merged_df['PROJECT_NAME'].apply(lambda x: ' '.join(x.split()[:5]) + ('...' if len(x.split()) > 5 else ''))
...
def plot_download_scatter(merged_df):
    ...
    fig.update_traces(
        marker=dict(color=DARK_RED, size=15),
...
def plot_network(df):
    ...
    for _, row in df_weights.iterrows():
        ...
        if not G.has_node(user):
            G.add_node(user, label='', color=MAGENTA, size=150) # IMPORTANT -- hide user_ids
        if not G.has_node(project):
            G.add_node(project, label=project, color=TEAL, size=250)

        # Add edges with weights
        G.add_edge(user, project, color = 'gray', weight=weight, title=f'Interactions: {weight}')
...
def network_legend():
    ...
    net.add_node('legend_user', label='Users', color=MAGENTA, size=5, x=-100, y=0, fixed=True, physics=False)
    net.add_node('legend_project', label='Projects', color=TEAL, size=10, x=0, y=0, fixed=True, physics=False)
    html_content = net.generate_html()
...
def plot_project_pageviews(df):
    ...
    """
    if df.empty:
        fig = px.bar(title="Check that query range falls within retention limit of 14 months and credentials are valid.")
        return fig
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title='Project',
            ticktext=[f"{text[:50]}..." if len(text) > 50 else text for text in df_sorted['PROJECT_NAME']],
    ...
    fig.update_traces(
        hovertemplate='<b>Project:</b> ' + df_sorted['PROJECT_NAME'] + '<br>' +
...
def plot_missing_institution(df, width=1600):
    ...
    fig = go.Figure(data=[
        go.Bar(
            ...
            x=df['PERCENTAGE'],
            orientation='h',
            marker_color=[PORTAL_BLUE if x == 'Valid' else ORANGE for x in df['ISSUE_TYPE']],
...
def plot_data_type_bar_chart(df, title="Data Types by Size", width=1400, height=600, color=PORTAL_BLUE):
    """
    ...
    fig = go.Figure(data=[
        go.Bar(
            ...
            y=df['SIZE_GB'],
            marker_color=color,
            text=[f"{size:.1f} GB<br>{count:,} files" 
...
def create_data_type_table(df):
    ...
    display_df['FORMATTED_SIZE'] = display_df.apply(
        lambda row: f"{row['SIZE_TB']:.2f} TB" if row['SIZE_TB'] >= 1 
