...
steps — e.g. running dbt — off the resolved clone name without re-deriving
snowclone's sanitization rules itself. dbt (or any other deploy tool) is
deliberately not part of the package: most databases only need schemachange, so
tool-specific steps like dbt belong in the calling workflow, keyed off `clone_db`.
