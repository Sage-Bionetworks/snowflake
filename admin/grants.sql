USE ROLE SECURITYADMIN;
-- ACCOUNTADMIN privileges
GRANT ROLE ACCOUNTADMIN
TO USER "x.schildwachter@sagebase.org";
GRANT ROLE ACCOUNTADMIN
TO USER "khai.do@sagebase.org";
GRANT ROLE ACCOUNTADMIN
TO USER "phil.snyder@sagebase.org";

-- SYSADMIN privileges
REVOKE ROLE SYSADMIN
FROM USER "kevin.boske@sagebase.org";
GRANT ROLE SYSADMIN
TO USER "x.schildwachter@sagebase.org";
GRANT ROLE SYSADMIN
TO USER "phil.snyder@sagebase.org";

-- Revoke unnecessary roles
-- This user only ever uses DATA_ANALYTICS and DATA_ENGINEER
REVOKE ROLE SYSADMIN
FROM USER DPE_SERVICE;
REVOKE ROLE SECURITYADMIN
FROM USER DPE_SERVICE;

-- warehouse usage privileges
GRANT USAGE ON WAREHOUSE COMPUTE_XSMALL
TO ROLE PUBLIC;
REVOKE USAGE ON WAREHOUSE COMPUTE_XSMALL
FROM ROLE DATA_ANALYTICS;
REVOKE USAGE ON WAREHOUSE COMPUTE_XSMALL
FROM ROLE GOVERNANCE;
REVOKE USAGE ON WAREHOUSE COMPUTE_XSMALL
FROM ROLE TECH_PRODUCT;
REVOKE USAGE ON WAREHOUSE COMPUTE_XSMALL
FROM ROLE SYNAPSE_DATA_WAREHOUSE_DEV_PROXY_ADMIN;
REVOKE USAGE ON WAREHOUSE COMPUTE_XSMALL
FROM ROLE SYNAPSE_DATA_WAREHOUSE_PROXY_ADMIN;

-- public grants
GRANT USAGE ON DATABASE SAGE
TO ROLE PUBLIC;

-- GENIE privileges
GRANT ROLE GENIE_ADMIN
TO ROLE SYSADMIN;
GRANT ROLE GENIE_ADMIN
TO USER "alexander.paynter@sagebase.org";
GRANT ROLE GENIE_ADMIN
TO USER "xindi.guo@sagebase.org";
GRANT ROLE GENIE_ADMIN
TO USER "chelsea.nayan@sagebase.org";
GRANT ROLE GENIE_ADMIN
TO USER "rixing.xu@sagebase.org";
GRANT ROLE GENIE_ADMIN
TO USER "adam.taylor@sagebase.org";
GRANT ROLE GENIE_ADMIN
TO USER GENIE_SERVICE;

-- AD privileges
GRANT ROLE AD TO ROLE SYSADMIN;
GRANT ROLE AD TO USER "samia.ahmed@sagebase.org";
GRANT ROLE AD TO USER "jo.scanlan@sagebase.org";
GRANT ROLE AD TO USER "karina.leal@sagebase.org";
GRANT ROLE AD TO USER "jessica.britton@sagebase.org";
GRANT ROLE AD TO USER "zoe.leanza@sagebase.org";
GRANT ROLE AD TO USER "milan.vu@sagebase.org";
GRANT ROLE AD TO USER "jessica.malenfant@sagebase.org";

-- NF privileges
GRANT ROLE SAGE_NF_ADMIN
TO ROLE SYSADMIN;
GRANT ROLE SAGE_NF_ADMIN
TO USER "anh.nguyet.vu@sagebase.org";
GRANT ROLE SAGE_NF_ADMIN
TO USER "thomas.yu@sagebase.org";

-- SCIDATA privileges
GRANT ROLE SCIDATA_ADMIN
TO ROLE SYSADMIN;
GRANT ROLE SCIDATA_ADMIN
TO USER "susheel.varma@sagebase.org";
GRANT ROLE SCIDATA_ADMIN
TO USER "thomas.yu@sagebase.org";

-- Data engineer privileges
GRANT ROLE DATA_ENGINEER
TO ROLE SYSADMIN;
GRANT ROLE DATA_ENGINEER
TO USER "phil.snyder@sagebase.org";
GRANT ROLE DATA_ENGINEER
TO USER "rixing.xu@sagebase.org";
GRANT ROLE DATA_ENGINEER
TO USER "thomas.yu@sagebase.org";
GRANT ROLE DATA_ENGINEER
TO USER "brad.macdonald@sagebase.org";
GRANT ROLE DATA_ENGINEER
TO USER "bryan.fauble@sagebase.org";
GRANT ROLE DATA_ENGINEER
TO USER "nick.grosenbacher@sagebase.org";
GRANT ROLE DATA_ENGINEER
TO USER "jenny.medina@sagebase.org";
GRANT ROLE DATA_ENGINEER
TO USER DPE_SERVICE;

-- data analytics privileges
GRANT ROLE DATA_ANALYTICS TO USER "diep.thach@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "rixing.xu@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "thomas.yu@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "anh.nguyet.vu@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "luca.foschini@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "xindi.guo@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "phil.snyder@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "chelsea.nayan@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "alexander.paynter@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "x.schildwachter@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "natosha.edmonds@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "kevin.boske@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "brad.macdonald@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "robert.allaway@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "victor.baham@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "meghasyam@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "pranav.anbarasu@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "elias.chaibub.neto@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "bryan.fauble@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "john.hill@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "bruce.hoff@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "marco.marasca@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "sandhra.sokhal@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "adam.hindman@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jessica.malenfant@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "ann.novakowski@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "christine.suver@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "adam.taylor@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "sophia.jobe@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "thomas.schaffter@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "solly.sieberts@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "arti.singh@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "dan.lu@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jessica.britton@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "christina.conrad@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "drew.duglan@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "zoe.leanza@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "anna.greenwood@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jay.hodgson@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "milan.vu@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "richard.yaxley@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "ashley.clayton@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "verena.chung@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jineta.banerjee@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jenny.medina@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "lakaija.johnson@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "sonia.carlson@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "anthony.williams@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "loren.wolfe@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "lingling.peng@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "gianna.jordan@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "mieko.hashimoto@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "andrew.lamb@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "milen.nikolov@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "amy.heiser@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "rchai@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "maria.diaz@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "gaia.andreoletti@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "susheel.varma@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "amber.nelson@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "tiara.adams@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "william.poehlman@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "alberto.pepe@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jessica.vera@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "kimberly.corrigan@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "mackenzie.wildman@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "aditi.gopalan@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "angie.bowen@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "aditya.nath@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "james.moon@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "orion.banks@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "lisa.pasquale@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "JONI.HARKER@SAGEBASE.ORG";
GRANT ROLE DATA_ANALYTICS TO USER "jo.scanlan@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "trisha.zintel@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "anthony.pena@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "hayley.sanchez@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "bishoy.kamel@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "andrea.varsavsky@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "serghei.mangul@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jaclyn.beck@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "ziwei.pan@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jessica.britton@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "karina.leal@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "ann.campton@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "savitha.sangameswaran@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "vanessa.barone@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jonathan.liaw-gray@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "laura.heath@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "melissa.klein@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "sarah.mansfield@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "andree-anne.berthiaume@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "dottie.young@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jordan.driscoll@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "ram.ayyala@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "beatriz.saldana@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "tera.derita@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "belinda.garana@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "emma.costa@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "julia.gray@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "luisa.chekrygin@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "laurielle.roberson@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jon.long@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "jessica.lundin@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "pranita.atri@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "amelia.kallaher@sagebase.org";

// Leaders
GRANT ROLE SAGE_LEADERS TO USER "kim.baggett@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "samuel.cason@sagebase.org";
GRANT ROLE DATA_ANALYTICS TO USER "amelia.weixler@sagebase.org";

// Sage Leadership
GRANT ROLE SAGE_LEADERS TO USER "luca.foschini@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "alberto.pepe@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "susheel.varma@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "christine.suver@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "mackenzie.wildman@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "amy.heiser@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "brandon.morgan@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "thomas.yu@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "milen.nikolov@sagebase.org";
GRANT ROLE SAGE_LEADERS TO USER "andrea.varsavsky@sagebase.org";

// Setting up the role hierarchy
GRANT ROLE SAGE_LEADERS TO ROLE SYSADMIN;
GRANT ROLE DATA_ANALYTICS TO ROLE SAGE_LEADERS;
GRANT ROLE DATA_ANALYTICS TO ROLE SCIDATA_ADMIN;
GRANT ROLE DATA_ANALYTICS TO ROLE SAGE_NF_ADMIN;
GRANT ROLE DATA_ANALYTICS TO ROLE GENIE_ADMIN;
GRANT ROLE DATA_ANALYTICS TO ROLE GOVERNANCE;

// FAIR
GRANT ROLE FAIR TO USER "anthony.williams@sagebase.org";
GRANT ROLE FAIR TO USER "loren.wolfe@sagebase.org";
GRANT ROLE FAIR TO USER "lingling.peng@sagebase.org";
GRANT ROLE FAIR TO USER "gianna.jordan@sagebase.org";
GRANT ROLE FAIR TO USER "mieko.hashimoto@sagebase.org";
GRANT ROLE FAIR TO USER "andrew.lamb@sagebase.org";
GRANT ROLE FAIR TO USER "milen.nikolov@sagebase.org";
GRANT ROLE FAIR TO USER "amy.heiser@sagebase.org";
GRANT ROLE FAIR TO ROLE SYSADMIN;

GRANT ROLE DPE_OPS TO USER "thomas.yu@sagebase.org";
GRANT ROLE DPE_OPS TO USER "sophia.jobe@sagebase.org";
GRANT ROLE DPE_OPS TO ROLE SYSADMIN;

GRANT ROLE GOVERNANCE TO USER "christine.suver@sagebase.org";
GRANT ROLE GOVERNANCE TO USER "kimberly.corrigan@sagebase.org";
GRANT ROLE GOVERNANCE TO USER "ann.novakowski@sagebase.org";
GRANT ROLE GOVERNANCE TO USER "anthony.pena@sagebase.org";
GRANT ROLE GOVERNANCE TO USER "samia.ahmed@sagebase.org";
GRANT ROLE GOVERNANCE TO USER "jonathan.liaw-gray@sagebase.org";
GRANT ROLE GOVERNANCE TO USER "samuel.cason@sagebase.org";
GRANT ROLE GOVERNANCE TO USER "amelia.weixler@sagebase.org";
GRANT ROLE GOVERNANCE TO ROLE SYSADMIN;

-- Create governance privileges
GRANT ROLE MASKING_ADMIN
TO USER "thomas.yu@sagebase.org";
USE ROLE ACCOUNTADMIN;
GRANT APPLY MASKING POLICY ON ACCOUNT
TO ROLE MASKING_ADMIN;


-- Synapse data warehouse privileges
GRANT USAGE ON INTEGRATION SYNAPSE_PROD_WAREHOUSE_S3
TO ROLE SYSADMIN;
GRANT USAGE ON INTEGRATION SYNAPSE_PROD_WAREHOUSE_S3
TO ROLE DATA_ENGINEER;

-- Grant storage integration privileges to DATA_ENGINEER, and <DATABASE>_PROXY_ADMIN
---- `DEV`
GRANT USAGE
	ON INTEGRATION SYNAPSE_SNAPSHOTS_DEV
	TO ROLE DATA_ENGINEER;
GRANT USAGE
    ON INTEGRATION SYNAPSE_SNAPSHOTS_DEV
    TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_PROXY_ADMIN;
---- `PROD`
GRANT USAGE
	ON INTEGRATION SYNAPSE_SNAPSHOTS_PROD
	TO ROLE DATA_ENGINEER;
GRANT USAGE
    ON INTEGRATION SYNAPSE_SNAPSHOTS_PROD
    TO ROLE SYNAPSE_DATA_WAREHOUSE_PROXY_ADMIN;

USE ROLE SECURITYADMIN;
GRANT CREATE MASKING POLICY ON SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
TO ROLE MASKING_ADMIN;

-- Allow for data engineer role to create tasks
-- https://docs.snowflake.com/en/user-guide/tasks-intro#creating-tasks
USE ROLE ACCOUNTADMIN;
GRANT EXECUTE TASK, EXECUTE MANAGED TASK ON ACCOUNT TO ROLE taskadmin;
GRANT CREATE DATABASE ON ACCOUNT TO ROLE DATA_ENGINEER; --noqa: PRS
USE ROLE securityadmin;
GRANT ROLE taskadmin TO ROLE DATA_ENGINEER; --noqa: PRS

-- Synapse data warehouse dev privileges
GRANT USAGE ON INTEGRATION SYNAPSE_DEV_WAREHOUSE_S3
TO ROLE SYSADMIN;

-- Narrowed from a blanket "ALL PROCEDURES IN DATABASE" grant to GOVERNANCE's actual usage:
-- just the LIST_DOWNLOADERS* procedures. The REVOKE is idempotent -- a no-op once the
-- blanket grant is gone -- and undoes the broad privilege GOVERNANCE previously held.
REVOKE USAGE ON ALL PROCEDURES IN DATABASE SYNAPSE_DATA_WAREHOUSE
FROM ROLE GOVERNANCE;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE.SYNAPSE.LIST_DOWNLOADERS(VARCHAR, VARCHAR)
TO ROLE GOVERNANCE;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE.SYNAPSE.LIST_DOWNLOADERS_DBG(DATE, VARCHAR)
TO ROLE GOVERNANCE;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE.SYNAPSE.LIST_DOWNLOADERS_WITH_SIZE(VARCHAR, VARCHAR)
TO ROLE GOVERNANCE;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE.SYNAPSE.LIST_DOWNLOADERS_WITH_SIZE_BY_MONTH(VARCHAR, VARCHAR)
TO ROLE GOVERNANCE;

-- DEV never had the blanket ALL PROCEDURES grant to GOVERNANCE; scope it to just
-- the LIST_DOWNLOADERS* procedures being mirrored here.
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE.LIST_DOWNLOADERS(VARCHAR, VARCHAR)
TO ROLE GOVERNANCE;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE.LIST_DOWNLOADERS_DBG(DATE, VARCHAR)
TO ROLE GOVERNANCE;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE.LIST_DOWNLOADERS_WITH_SIZE(VARCHAR, VARCHAR)
TO ROLE GOVERNANCE;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE.LIST_DOWNLOADERS_WITH_SIZE_BY_MONTH(VARCHAR, VARCHAR)
TO ROLE GOVERNANCE;

-- Grant USAGE on the LIST_DOWNLOADERS* procedures to developers (idempotent here since
-- CREATE OR REPLACE PROCEDURE drops existing grants; ownership transfer is handled separately
-- in admin/ownership_grants/V1.38.3__synapse_list_downloaders_procedures_ownership.sql)
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE.SYNAPSE.LIST_DOWNLOADERS(VARCHAR, VARCHAR)
TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_ALL_DEVELOPER;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE.SYNAPSE.LIST_DOWNLOADERS_DBG(DATE, VARCHAR)
TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_ALL_DEVELOPER;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE.SYNAPSE.LIST_DOWNLOADERS_WITH_SIZE(VARCHAR, VARCHAR)
TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_ALL_DEVELOPER;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE.SYNAPSE.LIST_DOWNLOADERS_WITH_SIZE_BY_MONTH(VARCHAR, VARCHAR)
TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_ALL_DEVELOPER;

GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE.LIST_DOWNLOADERS(VARCHAR, VARCHAR)
TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_ALL_DEVELOPER;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE.LIST_DOWNLOADERS_DBG(DATE, VARCHAR)
TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_ALL_DEVELOPER;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE.LIST_DOWNLOADERS_WITH_SIZE(VARCHAR, VARCHAR)
TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_ALL_DEVELOPER;
GRANT USAGE ON PROCEDURE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE.LIST_DOWNLOADERS_WITH_SIZE_BY_MONTH(VARCHAR, VARCHAR)
TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_ALL_DEVELOPER;

-- IP_INFO database privileges
REVOKE IMPORTED PRIVILEGES ON DATABASE IP_INFO
FROM ROLE DATA_ENGINEER;
REVOKE IMPORTED PRIVILEGES ON DATABASE IP_INFO
FROM ROLE GOVERNANCE;
REVOKE IMPORTED PRIVILEGES ON DATABASE IP_INFO
FROM ROLE DATA_ANALYTICS;
GRANT IMPORTED PRIVILEGES ON DATABASE IP_INFO
TO ROLE PUBLIC;

-- Sage database privileges
GRANT USAGE ON DATABASE SAGE
TO ROLE DATA_ENGINEER;
GRANT SELECT ON FUTURE TABLES IN SCHEMA SAGE.IT
TO ROLE DATA_ANALYTICS;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA SAGE.IT
TO ROLE DATA_ANALYTICS;
GRANT USAGE
	ON SCHEMA SAGE.IT
	TO ROLE SAGE_PRODUCT_ADMIN;
GRANT SELECT, REFERENCES
	ON ALL TABLES IN SCHEMA SAGE.IT
	TO ROLE SAGE_PRODUCT_ADMIN;
GRANT USAGE ON DATABASE SAGE
TO ROLE DATA_ANALYTICS;

-- SAGE.AUDIT
-- Standardize RBAC and grants with analyst schema
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_AUDIT_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_AUDIT_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.AUDIT
	TO ROLE SAGE_AUDIT_ANALYST;
GRANT SELECT, REFERENCES
	ON ALL TABLES IN SCHEMA SAGE.AUDIT
	TO ROLE SAGE_AUDIT_ANALYST;
GRANT SELECT
	ON ALL VIEWS IN SCHEMA SAGE.AUDIT
	TO ROLE SAGE_AUDIT_ANALYST;
GRANT ROLE SAGE_AUDIT_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_AUDIT_ANALYST
	TO ROLE GOVERNANCE;
GRANT ROLE SAGE_AUDIT_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_AUDIT_ADMIN;

-- AD database privileges
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_AD_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_AD_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.AD
	TO ROLE SAGE_AD_ANALYST;
GRANT SELECT, REFERENCES
	ON ALL TABLES IN SCHEMA SAGE.AD
	TO ROLE SAGE_AD_ANALYST;
GRANT SELECT
	ON ALL VIEWS IN SCHEMA SAGE.AD
	TO ROLE SAGE_AD_ANALYST;
GRANT USAGE
	ON ALL FILE FORMATS IN SCHEMA SAGE.AD
	TO ROLE SAGE_AD_ANALYST;
GRANT USAGE
	ON ALL FUNCTIONS IN SCHEMA SAGE.AD
	TO ROLE SAGE_AD_ANALYST;
GRANT USAGE
	ON ALL PROCEDURES IN SCHEMA SAGE.AD
	TO ROLE SAGE_AD_ANALYST;
GRANT READ, USAGE
	ON ALL STAGES IN SCHEMA SAGE.AD
	TO ROLE SAGE_AD_ANALYST;
GRANT ROLE SAGE_AD_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SAGE_AD_ANALYST
	TO ROLE AD;
GRANT ROLE SAGE_AD_ADMIN
	TO ROLE AD;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_AD_ADMIN;

-- SCIDATA database privileges
GRANT ALL PRIVILEGES ON SCHEMA SAGE.SCIDATA
TO ROLE SCIDATA_ADMIN;



-- SYNAPSE_DATA_WAREHOUSE domain roles
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ADMIN TO ROLE SYSADMIN;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST TO ROLE SYNAPSE_DATA_WAREHOUSE_ADMIN;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST TO ROLE AD;

-- SYNAPSE_DATA_WAREHOUSE_DEV domain roles
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_DEV_ADMIN TO ROLE SYSADMIN;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_DEV_ANALYST TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_ADMIN;

-- CREATE DATABASE privileges on domain ADMIN roles
USE ROLE ACCOUNTADMIN;
GRANT CREATE DATABASE ON ACCOUNT TO ROLE SYNAPSE_DATA_WAREHOUSE_ADMIN;
GRANT CREATE DATABASE ON ACCOUNT TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_ADMIN;
-- Switch back to SECURITYADMIN role
USE ROLE SECURITYADMIN;

---- RBAC reconfiguration of data warehouse ----
-- The following grants provide read access on most
-- objects presently within the data warehouse.

---- SYNAPSE_DATA_WAREHOUSE ----
-- Grant the aggregate roles usage on their respective schema
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_ALL_ANALYST;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.SCHEMACHANGE
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SCHEMACHANGE_ALL_ANALYST;

-- USAGE/MONITOR on schema RDS_LANDING
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.RDS_LANDING
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.RDS_LANDING_ALL_DEVELOPER;

-- USAGE/MONITOR on schema RDS_RAW
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.RDS_RAW
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.RDS_RAW_ALL_DEVELOPER;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.RDS_RAW
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.RDS_RAW_ALL_ANALYST;

-- Grant object-type-specific privileges to the appropriate object-type-specific role
-- SYNAPSE
GRANT SELECT, REFERENCES
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_TABLE_READ;
GRANT SELECT, MONITOR
	ON ALL DYNAMIC TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_TABLE_READ;
GRANT USAGE, READ
	ON ALL STAGES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_STAGE_READ;
GRANT SELECT, REFERENCES
	ON ALL VIEWS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_VIEW_READ;
GRANT MONITOR
	ON ALL TASKS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_TASK_READ;

-- SYNAPSE_RAW
GRANT SELECT, REFERENCES
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW_TABLE_READ;
GRANT USAGE, READ
	ON ALL STAGES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW_STAGE_READ;
GRANT SELECT
	ON ALL STREAMS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW_STREAM_READ;
GRANT MONITOR
	ON ALL TASKS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW_TASK_READ;

-- RDS_LANDING
GRANT MONITOR
	ON ALL TASKS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.RDS_LANDING
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.RDS_LANDING_TASK_READ;

-- SCHEMACHANGE
GRANT SELECT, REFERENCES
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SCHEMACHANGE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SCHEMACHANGE_TABLE_READ;

---- SYNAPSE_DATA_WAREHOUSE_DEV ----
-- Grant the aggregate roles usage on their respective schema
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_ALL_ANALYST;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SCHEMACHANGE
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SCHEMACHANGE_ALL_ANALYST;

-- USAGE/MONITOR on schema RDS_LANDING
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.RDS_LANDING
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.RDS_LANDING_ALL_DEVELOPER;

-- USAGE/MONITOR on schema RDS_RAW
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW_ALL_DEVELOPER;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW_ALL_ANALYST;

-- Grant object-type-specific privileges to the appropriate object-type-specific role
-- SYNAPSE
GRANT SELECT, REFERENCES
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_TABLE_READ;
GRANT SELECT, MONITOR
	ON ALL DYNAMIC TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_TABLE_READ;

-- SYNAPSE_RAW
GRANT SELECT, REFERENCES
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW_TABLE_READ;
GRANT USAGE, READ
	ON ALL STAGES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW_STAGE_READ;
GRANT SELECT
	ON ALL STREAMS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW_STREAM_READ;
GRANT MONITOR
	ON ALL TASKS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW_TASK_READ;

-- RDS_LANDING
GRANT MONITOR
	ON ALL TASKS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.RDS_LANDING
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.RDS_LANDING_TASK_READ;

-- SCHEMACHANGE
GRANT SELECT, REFERENCES
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SCHEMACHANGE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SCHEMACHANGE_TABLE_READ;

-- RDS_RAW

-- backfill SELECT, MONITOR grants for existing dynamic tables for the table-read role
-- (future dynamic tables are covered by admin/future_grants/V1.37.1__rds_raw_future_dynamic_tables.sql)
GRANT SELECT, MONITOR
	ON ALL DYNAMIC TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW_TABLE_READ;

-- backfill SELECT, MONITOR grants for existing dynamic tables for the table-read-masked role
-- (future dynamic tables are covered by admin/future_grants/V1.37.0__rds_raw_future_tables_masked_read.sql)
GRANT SELECT, MONITOR
	ON ALL DYNAMIC TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW_TABLE_READ_MASKED;

-- backfill SELECT, MONITOR grants for existing dynamic tables for the table-read-masked role in prod
-- (needed because the future dynamic tables grant for this role isn't established on prod until
-- admin/future_grants/ runs, which happens after synapse_data_warehouse/ creates the tables in the
-- same deploy that promotes RDS_RAW_TABLE_READ_MASKED from dev to prod)
GRANT SELECT, MONITOR
	ON ALL DYNAMIC TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.RDS_RAW
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.RDS_RAW_TABLE_READ_MASKED;

-- Allow SECURITYADMIN to deploy schemachange for versioned admin scripts
GRANT USAGE
    ON DATABASE METADATA
    TO ROLE SECURITYADMIN;
GRANT USAGE
    ON SCHEMA METADATA.SCHEMACHANGE
    TO ROLE SECURITYADMIN;
GRANT SELECT, INSERT
    ON TABLE METADATA.SCHEMACHANGE.CHANGE_HISTORY
    TO ROLE SECURITYADMIN;

-- Grant appropriate roles to our service accounts
GRANT ROLE ACCOUNTADMIN
	TO USER ADMIN_SERVICE;
GRANT ROLE DATA_ENGINEER
	TO USER DEVELOPER_SERVICE;

-- Allow the proxy admins to run serverless tasks
USE ROLE ACCOUNTADMIN;
GRANT EXECUTE MANAGED TASK
	ON ACCOUNT
	TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_PROXY_ADMIN;
GRANT EXECUTE MANAGED TASK
	ON ACCOUNT
	TO ROLE SYNAPSE_DATA_WAREHOUSE_PROXY_ADMIN;
USE ROLE SECURITYADMIN;

-- Grant usage of our warehouses to the proxy admins
GRANT USAGE ON WAREHOUSE COMPUTE_MEDIUM
	TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_PROXY_ADMIN;
GRANT USAGE ON WAREHOUSE COMPUTE_MEDIUM
	TO ROLE SYNAPSE_DATA_WAREHOUSE_PROXY_ADMIN;

-- Allow the proxy admins to run tasks (generally)
USE ROLE ACCOUNTADMIN;
GRANT EXECUTE TASK
	ON ACCOUNT
	TO ROLE SYNAPSE_DATA_WAREHOUSE_PROXY_ADMIN;
GRANT EXECUTE TASK
	ON ACCOUNT
	TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_PROXY_ADMIN;
-- Allow SAGE_CCKP_ADMIN to schedule dynamic table refreshes
GRANT EXECUTE MANAGED TASK
	ON ACCOUNT
	TO ROLE SAGE_CCKP_ADMIN;
USE ROLE SECURITYADMIN;

-- Allow the proxy admin to use storage integrations
-- necessary for external stages
USE ROLE ACCOUNTADMIN;
GRANT USAGE
    ON INTEGRATION SYNAPSE_PROD_WAREHOUSE_S3
    TO ROLE SYNAPSE_DATA_WAREHOUSE_PROXY_ADMIN;
GRANT USAGE
    ON INTEGRATION SYNAPSE_DEV_WAREHOUSE_S3
    TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_PROXY_ADMIN;
USE ROLE SECURITYADMIN;

USE ROLE ACCOUNTADMIN;
GRANT USAGE
    ON INTEGRATION SLACK_INGEST_UPDATES
    TO ROLE SYNAPSE_DATA_WAREHOUSE_PROXY_ADMIN;
GRANT USAGE
    ON INTEGRATION DEV_SLACK_INGEST_UPDATES
    TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_PROXY_ADMIN;
USE ROLE SECURITYADMIN;

-- Supplant read-type privileges on data warehouse --
-- We first grant analyst account roles USAGE of the
-- respective "core" analyst account role for the data
-- warehouse. We then revoke redundant or excessive (e.g.,
-- write-type) privileges from the same account roles.

-- SYNAPSE_DATA_WAREHOUSE_DEV --
-- Only DATA_ENGINEER needs read-type privileges on DEV
-- No action required since read-type privileges propagate
-- via *ALL_DEVELOPER database roles.

-- SYNAPSE_DATA_WAREHOUSE --
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE GOVERNANCE;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE AD;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE DATA_ANALYTICS;

-- Revoke DATA_ANALYTICS grants;
-- The core analyst role should be granted to team-specific account roles
-- as shown in this design: https://sagebionetworks.jira.com/wiki/spaces/DPE/pages/4076863503/Role+and+Privilege+Management#Implementation-examples
REVOKE ROLE DATA_ANALYTICS
	FROM ROLE AD;

-- Revoke grants redundant with SYNAPSE_DATA_WAREHOUSE[_DEV]_ANALYST
-- SYNAPSE_DATA_WAREHOUSE --
REVOKE CREATE STREAMLIT, CREATE PROCEDURE, CREATE STAGE, CREATE STREAM, CREATE TABLE, CREATE TASK, MODIFY, USAGE, CREATE FUNCTION
	ON ALL SCHEMAS
	IN DATABASE SYNAPSE_DATA_WAREHOUSE
	FROM ROLE DATA_ENGINEER;
REVOKE CREATE SCHEMA, USAGE
	ON DATABASE SYNAPSE_DATA_WAREHOUSE
	FROM ROLE DATA_ENGINEER;
REVOKE USAGE
	ON DATABASE SYNAPSE_DATA_WAREHOUSE
	FROM ROLE GOVERNANCE;
REVOKE USAGE
	ON ALL SCHEMAS
	IN DATABASE SYNAPSE_DATA_WAREHOUSE
	FROM ROLE DATA_ANALYTICS;
REVOKE USAGE
	ON DATABASE SYNAPSE_DATA_WAREHOUSE
	FROM ROLE DATA_ANALYTICS;
REVOKE USAGE
	ON ALL SCHEMAS
	IN DATABASE SYNAPSE_DATA_WAREHOUSE
	FROM ROLE AD;
REVOKE USAGE
	ON ALL SCHEMAS
	IN DATABASE SYNAPSE_DATA_WAREHOUSE
	FROM ROLE GOVERNANCE;
REVOKE CREATE SCHEMA, USAGE
	ON DATABASE SYNAPSE_DATA_WAREHOUSE
	FROM ROLE AD;

-- SYNAPSE_DATA_WAREHOUSE.SCHEMACHANGE --
REVOKE EVOLVE SCHEMA, UPDATE, SELECT, INSERT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SCHEMACHANGE
	FROM ROLE DATA_ENGINEER;
REVOKE SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SCHEMACHANGE
	FROM ROLE GOVERNANCE;

-- SYNAPSE_DATA_WAREHOUSE.SYNAPSE --
REVOKE SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	FROM ROLE DATA_ANALYTICS;
REVOKE SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	FROM ROLE AD;
REVOKE OPERATE, SELECT, MONITOR
	ON ALL DYNAMIC TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	FROM ROLE DATA_ENGINEER;
REVOKE EVOLVE SCHEMA, INSERT, UPDATE, SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	FROM ROLE DATA_ENGINEER;
REVOKE SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	FROM ROLE GOVERNANCE;
REVOKE SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	FROM ROLE PUBLIC;

-- SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW --
REVOKE EVOLVE SCHEMA, SELECT, UPDATE, INSERT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	FROM ROLE DATA_ENGINEER;
REVOKE APPLYBUDGET, MONITOR, OPERATE
	ON ALL TASKS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	FROM ROLE DATA_ENGINEER;
REVOKE SELECT
	ON ALL STREAMS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	FROM ROLE DATA_ENGINEER;
REVOKE SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	FROM ROLE GOVERNANCE;
REVOKE SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
	FROM ROLE DATA_ANALYTICS;

-- SYNAPSE_DATA_WAREHOUSE_DEV --
REVOKE CREATE TABLE, CREATE STREAM, CREATE TASK, MODIFY, USAGE, CREATE STAGE, CREATE PROCEDURE, CREATE FUNCTION, CREATE STREAMLIT
	ON ALL SCHEMAS
	IN DATABASE SYNAPSE_DATA_WAREHOUSE_DEV
	FROM ROLE DATA_ENGINEER;
REVOKE USAGE, CREATE SCHEMA
	ON DATABASE SYNAPSE_DATA_WAREHOUSE_DEV
	FROM ROLE DATA_ENGINEER;

-- SYNAPSE_DATA_WAREHOUSE_DEV.SCHEMACHANGE --
REVOKE INSERT, SELECT, UPDATE, EVOLVE SCHEMA
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SCHEMACHANGE
	FROM ROLE DATA_ENGINEER;

-- SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE --
REVOKE OPERATE, SELECT, MONITOR
	ON ALL DYNAMIC TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE
	FROM ROLE DATA_ENGINEER;
REVOKE UPDATE, INSERT, EVOLVE SCHEMA, SELECT
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE
	FROM ROLE DATA_ENGINEER;

-- SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW --
REVOKE SELECT
	ON ALL STREAMS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW
	FROM ROLE DATA_ENGINEER;
REVOKE EVOLVE SCHEMA, INSERT, SELECT, UPDATE
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW
	FROM ROLE DATA_ENGINEER;
REVOKE APPLYBUDGET, MONITOR, OPERATE
	ON ALL TASKS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW
	FROM ROLE DATA_ENGINEER;

-- Grant privileges previously given to SYNAPSE_RAW_ALL_ANALYST
-- to SYNAPSE_RAW_ALL_DEVELOPER
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_RAW_ALL_DEVELOPER;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_RAW_ALL_DEVELOPER;

-- Grant schema privileges for SYNAPSE_AGGREGATE schema
-- SYNAPSE_DATA_WAREHOUSE
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_AGGREGATE
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_AGGREGATE_ALL_ANALYST;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_AGGREGATE
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_AGGREGATE_ALL_DEVELOPER;
-- SYNAPSE_DATA_WAREHOUSE_DEV
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_AGGREGATE
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_AGGREGATE_ALL_ANALYST;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_AGGREGATE
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_AGGREGATE_ALL_DEVELOPER;

-- Grant table read privileges in SYNAPSE_AGGREGATE schema
-- for initial "test" table
-- SYNAPSE_DATA_WAREHOUSE
GRANT SELECT, REFERENCES
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_AGGREGATE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_AGGREGATE_TABLE_READ;
-- SYNAPSE_DATA_WAREHOUSE_DEV
GRANT SELECT, REFERENCES
	ON ALL TABLES
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_AGGREGATE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_AGGREGATE_TABLE_READ;

-- Grant non-database-specific account roles inheritance of database roles
-- SYNAPSE_DATA_WAREHOUSE
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_AGGREGATE_ALL_ANALYST
    TO ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_AGGREGATE_ALL_DEVELOPER
    TO ROLE DATA_ENGINEER;
-- SYNAPSE_DATA_WAREHOUSE_DEV
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_AGGREGATE_ALL_ANALYST
    TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_ANALYST;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_AGGREGATE_ALL_DEVELOPER
    TO ROLE DATA_ENGINEER;

-- Grant schema privileges for SYNAPSE_EVENT schema
-- SYNAPSE_DATA_WAREHOUSE
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_ALL_ANALYST;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_ALL_DEVELOPER;
-- SYNAPSE_DATA_WAREHOUSE_DEV
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_ALL_ANALYST;
GRANT USAGE, MONITOR
    ON SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_ALL_DEVELOPER;

-- Grant table-read in SYNAPSE_EVENT schema
-- SYNAPSE_DATA_WAREHOUSE
GRANT SELECT, REFERENCES
    ON ALL TABLES
    IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_TABLE_READ;
-- SYNAPSE_DATA_WAREHOUSE_DEV
GRANT SELECT, REFERENCES
    ON ALL TABLES
    IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT
    TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_TABLE_READ;

-- Inherit event roles into your account roles
-- SYNAPSE_DATA_WAREHOUSE
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_ALL_ANALYST
    TO ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_ALL_DEVELOPER
    TO ROLE DATA_ENGINEER;
-- SYNAPSE_DATA_WAREHOUSE_DEV
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_ALL_ANALYST
    TO ROLE SYNAPSE_DATA_WAREHOUSE_DEV_ANALYST;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_ALL_DEVELOPER
    TO ROLE DATA_ENGINEER;

-- Grant SYNAPSE_EVENT_GOVERNANCE_READ to governance-related account roles
-- SYNAPSE_DATA_WAREHOUSE
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_GOVERNANCE_READ
    TO ROLE GOVERNANCE;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_GOVERNANCE_READ
    TO ROLE SAGE_LEADERS;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_GOVERNANCE_READ
    TO ROLE DATA_ENGINEER;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_GOVERNANCE_READ
	TO ROLE TECH_PRODUCT;
-- SYNAPSE_DATA_WAREHOUSE_DEV
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_GOVERNANCE_READ
    TO ROLE GOVERNANCE;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_GOVERNANCE_READ
    TO ROLE SAGE_LEADERS;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_GOVERNANCE_READ
    TO ROLE DATA_ENGINEER;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_GOVERNANCE_READ
	TO ROLE TECH_PRODUCT;

-- Grant RDS_RAW_ALL_ANALYST (masked RDS_RAW read access) to TECH_PRODUCT/SAGE_LEADERS
-- SYNAPSE_DATA_WAREHOUSE
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.RDS_RAW_ALL_ANALYST
	TO ROLE TECH_PRODUCT;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.RDS_RAW_ALL_ANALYST
	TO ROLE SAGE_LEADERS;
-- SYNAPSE_DATA_WAREHOUSE_DEV
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW_ALL_ANALYST
	TO ROLE TECH_PRODUCT;
GRANT DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.RDS_RAW_ALL_ANALYST
	TO ROLE SAGE_LEADERS;

-- The proxy admin will own the task which creates our backup
USE ROLE accountadmin;
GRANT CREATE DATABASE
	ON ACCOUNT
	TO ROLE synapse_data_warehouse_proxy_admin;

-- Revoke USAGE on GOOGLE_ANALYTICS database from the DATA_ANALYTICS
-- role via the application role DATA_READER (in the GOOGLE_ANALYTICS share)
-- For reasons which I don't entirely understand, SECURITYADMIN is not able to
-- see this role despite having the MANAGE GRANTS privilege, although the
-- role's owner, ACCOUNTADMIN, is able to.
REVOKE APPLICATION ROLE GOOGLE_ANALYTICS.DATA_READER
  FROM ROLE DATA_ANALYTICS;
USE ROLE securityadmin;

-- Grant privileges related to function access in SYNAPSE schema
GRANT USAGE
	ON ALL FUNCTIONS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_FUNCTION_READ;
GRANT USAGE
	ON ALL FUNCTIONS
	IN SCHEMA SYNAPSE_DATA_WAREHOUSE.SYNAPSE
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_FUNCTION_READ;

-- Conform `SAGE.CITATIONS` to standard analyst schema privilege model
-- DB/Schema
GRANT USAGE
	ON DATABASE SAGE
	TO ROLE SAGE_CITATIONS_ADMIN;
GRANT USAGE
	ON DATABASE SAGE
	TO ROLE SAGE_CITATIONS_ANALYST;
GRANT USAGE
	ON SCHEMA SAGE.CITATIONS
	TO ROLE SAGE_CITATIONS_ANALYST;
-- Tables
GRANT SELECT, REFERENCES
	ON TABLE SAGE.CITATIONS.AD_GOOGLE_SCHOLAR
	TO ROLE SAGE_CITATIONS_ANALYST;
GRANT SELECT, REFERENCES
	ON TABLE SAGE.CITATIONS.GENIE
	TO ROLE SAGE_CITATIONS_ANALYST;
GRANT SELECT, REFERENCES
	ON TABLE SAGE.CITATIONS.GENIE_GOOGLE_SCHOLAR
	TO ROLE SAGE_CITATIONS_ANALYST;
GRANT SELECT, REFERENCES
	ON TABLE SAGE.CITATIONS.SYNAPSE_MINTED_DOIS
	TO ROLE SAGE_CITATIONS_ANALYST;
-- Role inheritance
GRANT ROLE SAGE_CITATIONS_ANALYST
	TO ROLE DATA_ANALYTICS;
GRANT ROLE SAGE_CITATIONS_ANALYST
	TO ROLE DATA_ENGINEER;

-- Conform `SAGE.GOVERNANCE` to standard analyst schema privilege model
-- DB/Schema
GRANT USAGE
	ON DATABASE SAGE
	TO ROLE SAGE_GOVERNANCE_ADMIN;
GRANT USAGE
	ON DATABASE SAGE
	TO ROLE SAGE_GOVERNANCE_ANALYST;
GRANT USAGE
	ON SCHEMA SAGE.GOVERNANCE
	TO ROLE SAGE_GOVERNANCE_ANALYST;
-- Role inheritance
GRANT ROLE SAGE_GOVERNANCE_ANALYST
	TO ROLE GOVERNANCE;
GRANT ROLE SAGE_GOVERNANCE_ANALYST
	TO ROLE SAGE_LEADERS;
GRANT ROLE SAGE_GOVERNANCE_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SAGE_GOVERNANCE_ANALYST
	TO ROLE TECH_PRODUCT;
GRANT ROLE SAGE_GOVERNANCE_ANALYST
	TO ROLE DATA_ANALYTICS;

-- Streamlit app grants
GRANT USAGE ON STREAMLIT SAGE.NF.NF_USAGE_METRICS
	TO ROLE SAGE_NF_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.CCKP.MC2_CENTER_METRICS
	TO ROLE SAGE_CCKP_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.ACL_AUDIT
	TO ROLE SAGE_PRODUCT_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.AD.MODEL_AD_USAGE_METRICS
	TO ROLE SAGE_AD_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.ARK.ARK_USAGE_METRICS
	TO ROLE SAGE_ARK_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.AMP_ALS.AMP_ALS_USAGE_METRICS
	TO ROLE SAGE_AMP_ALS_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.SYNAPSE_MCP_USAGE
	TO ROLE SAGE_PRODUCT_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.STRIDES_COVERED_BUCKETS
	TO ROLE SAGE_PRODUCT_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.PYTHON_AND_COMMAND_LINE_CLIENT_USAGE
	TO ROLE SAGE_PRODUCT_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.GENIE.GENIE_USAGE_METRICS
	TO ROLE SAGE_GENIE_ANALYST;
REVOKE USAGE ON STREAMLIT SAGE.GOVERNANCE.DATA_ACCESS_DASHBOARD
	FROM ROLE DATA_ENGINEER;
GRANT USAGE ON STREAMLIT SAGE.GOVERNANCE.DATA_ACCESS_DASHBOARD
	TO ROLE SAGE_GOVERNANCE_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.SYNAPSE_PERFORMANCE_METRICS
	TO ROLE SAGE_PRODUCT_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.ACL_AUDIT
	TO ROLE DATA_ANALYTICS;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.SYNAPSE_MCP_USAGE
	TO ROLE DATA_ANALYTICS;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.STRIDES_COVERED_BUCKETS
	TO ROLE DATA_ANALYTICS;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.PYTHON_AND_COMMAND_LINE_CLIENT_USAGE
	TO ROLE DATA_ANALYTICS;
GRANT USAGE ON STREAMLIT SAGE.PRODUCT.SYNAPSE_PERFORMANCE_METRICS
	TO ROLE DATA_ANALYTICS;
GRANT USAGE ON STREAMLIT SAGE.AD.ADKP_METRICS
	TO ROLE SAGE_AD_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.AUDIT.AUDIT
	TO ROLE SAGE_AUDIT_ANALYST;
GRANT USAGE ON STREAMLIT SAGE.ELITE.ELITE_USAGE_METRICS
	TO ROLE SAGE_ELITE_ANALYST;

-- Allow the admin role to own and refresh dynamic tables
-- sourced from `synapse_data_warehouse`
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_GOVERNANCE_ADMIN;

-- Conform new SAGE analyst schemas to standard analyst schema privilege model
-- SAGE.NF
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_NF_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_NF_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.NF
	TO ROLE SAGE_NF_ANALYST;
GRANT ROLE SAGE_NF_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_NF_ADMIN;

-- SAGE.GENIE
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_GENIE_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_GENIE_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.GENIE
	TO ROLE SAGE_GENIE_ANALYST;
GRANT ROLE SAGE_GENIE_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_GENIE_ADMIN;
GRANT ROLE SAGE_CITATIONS_ANALYST
	TO ROLE SAGE_GENIE_ADMIN;

-- SAGE.ARK
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_ARK_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_ARK_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.ARK
	TO ROLE SAGE_ARK_ANALYST;
GRANT ROLE SAGE_ARK_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_ARK_ADMIN;

-- SAGE.AMP_ALS
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_AMP_ALS_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_AMP_ALS_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.AMP_ALS
	TO ROLE SAGE_AMP_ALS_ANALYST;
GRANT ROLE SAGE_AMP_ALS_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_AMP_ALS_ADMIN;

-- SAGE.PRODUCT
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_PRODUCT_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_PRODUCT_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.PRODUCT
	TO ROLE SAGE_PRODUCT_ANALYST;
GRANT ROLE SAGE_PRODUCT_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SAGE_PRODUCT_ANALYST
	TO ROLE TECH_PRODUCT;
GRANT ROLE SAGE_PRODUCT_ANALYST
	TO ROLE DATA_ANALYTICS;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_PRODUCT_ADMIN;

-- SAGE.CCKP
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_CCKP_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_CCKP_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.CCKP
	TO ROLE SAGE_CCKP_ANALYST;
GRANT ROLE SAGE_CCKP_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_CCKP_ADMIN;

-- SAGE.ELITE
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_ELITE_ADMIN;
GRANT USAGE, MONITOR
	ON DATABASE SAGE
	TO ROLE SAGE_ELITE_ANALYST;
GRANT USAGE, MONITOR
	ON SCHEMA SAGE.ELITE
	TO ROLE SAGE_ELITE_ANALYST;
GRANT ROLE SAGE_ELITE_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
	TO ROLE SAGE_ELITE_ADMIN;

-- Sage DB related role inheritance
GRANT ROLE SAGE_ADMIN
	TO ROLE SYSADMIN;
GRANT ROLE SAGE_CITATIONS_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_GOVERNANCE_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_NF_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_GENIE_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_ARK_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_AMP_ALS_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_PRODUCT_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_CCKP_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_AD_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_ELITE_ADMIN
	TO ROLE SAGE_ADMIN;
GRANT ROLE SAGE_AUDIT_ADMIN
	TO ROLE SAGE_ADMIN;

-- GOOGLE_ANALYTICS grants
-- The usual DB admin > schema admin
GRANT ROLE GOOGLE_ANALYTICS_AGGREGATE_ADMIN
	TO ROLE SAGE_ADMIN;
-- Grant the analyst role to our developers and analysts
GRANT ROLE GOOGLE_ANALYTICS_AGGREGATE_ANALYST
	TO ROLE DATA_ENGINEER;
GRANT ROLE GOOGLE_ANALYTICS_AGGREGATE_ANALYST
	TO ROLE DATA_ANALYTICS;

-- TECH_PRODUCT: read access to SYNAPSE_RDS_SNAPSHOT (censored)
GRANT USAGE ON DATABASE SYNAPSE_RDS_SNAPSHOT
    TO ROLE TECH_PRODUCT;
GRANT DATABASE ROLE SYNAPSE_RDS_SNAPSHOT.raw_table_read_censored
    TO ROLE TECH_PRODUCT;

-- SAGE_PRODUCT_ADMIN: read access to SYNAPSE_RDS_SNAPSHOT (censored)
GRANT USAGE ON DATABASE SYNAPSE_RDS_SNAPSHOT
    TO ROLE SAGE_PRODUCT_ADMIN;
GRANT DATABASE ROLE SYNAPSE_RDS_SNAPSHOT.raw_table_read_censored
    TO ROLE SAGE_PRODUCT_ADMIN;

-- TECH_PRODUCT: read access to SYNAPSE_DATA_WAREHOUSE
GRANT ROLE SYNAPSE_DATA_WAREHOUSE_ANALYST
    TO ROLE TECH_PRODUCT;

-- TECH_PRODUCT user grants
GRANT ROLE TECH_PRODUCT TO USER "christina.parry@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "kevin.boske@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "milen.nikolov@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "ann.novakowski@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "samia.ahmed@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "sophia.jobe@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "shaun.kalweit@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "thomas.yu@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "phil.snyder@sagebase.org";
GRANT ROLE TECH_PRODUCT TO USER "jenny.medina@sagebase.org";

-- Warehouse access for Streamlit apps in SAGE.GOVERNANCE
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_GOVERNANCE_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_GOVERNANCE_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE DATA_ENGINEER;
GRANT MONITOR ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE DATA_ENGINEER;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE TECH_PRODUCT;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_ARK_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_ARK_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_NF_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_NF_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_AMP_ALS_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_AMP_ALS_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_GENIE_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_GENIE_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_PRODUCT_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_PRODUCT_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_CCKP_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_CCKP_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_AD_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_AD_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_ELITE_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_ELITE_ADMIN;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_AUDIT_ANALYST;
GRANT USAGE ON WAREHOUSE STREAMLIT_XSMALL
	TO ROLE SAGE_AUDIT_ADMIN;

-- SAGE.ARK analyst user grants
GRANT ROLE SAGE_ARK_ANALYST TO USER "jessica.vera@sagebase.org";
GRANT ROLE SAGE_ARK_ANALYST TO USER "bishoy.kamel@sagebase.org";

-- SAGE.NF analyst user grants
GRANT ROLE SAGE_NF_ANALYST TO USER "james.moon@sagebase.org";
GRANT ROLE SAGE_NF_ANALYST TO USER "aditya.nath@sagebase.org";
GRANT ROLE SAGE_NF_ANALYST TO USER "robert.allaway@sagebase.org";
GRANT ROLE SAGE_NF_ANALYST TO USER "belinda.garana@sagebase.org";

-- SAGE.AMP_ALS analyst user grants
GRANT ROLE SAGE_AMP_ALS_ANALYST TO USER "jessica.vera@sagebase.org";
GRANT ROLE SAGE_AMP_ALS_ANALYST TO USER "ram.ayyala@sagebase.org";
GRANT ROLE SAGE_AMP_ALS_ANALYST TO USER "vanessa.barone@sagebase.org";

-- SAGE.GENIE analyst user grants
GRANT ROLE SAGE_GENIE_ANALYST TO USER "adam.taylor@sagebase.org";
GRANT ROLE SAGE_GENIE_ANALYST TO USER "ashley.clayton@sagebase.org";

-- SAGE.ELITE analyst user grants
GRANT ROLE SAGE_ELITE_ANALYST TO USER "milan.vu@sagebase.org";
GRANT ROLE SAGE_ELITE_ANALYST TO USER "melissa.klein@sagebase.org";

-- SAGE.CCKP user grants
GRANT ROLE SAGE_CCKP_ADMIN TO USER "verena.chung@sagebase.org";
GRANT ROLE SAGE_CCKP_ANALYST TO USER "ashley.clayton@sagebase.org";
GRANT ROLE SAGE_CCKP_ANALYST TO USER "savitha.sangameswaran@sagebase.org";
GRANT ROLE SAGE_CCKP_ANALYST TO USER "jineta.banerjee@sagebase.org";
GRANT ROLE SAGE_CCKP_ANALYST TO USER "ziwei.pan@sagebase.org";
GRANT ROLE SAGE_CCKP_ANALYST TO USER "amber.nelson@sagebase.org";

-- SAGE.AD user grants
GRANT ROLE SAGE_AD_ANALYST TO USER "samia.ahmed@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "jo.scanlan@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "karina.leal@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "jessica.britton@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "zoe.leanza@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "milan.vu@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "jessica.malenfant@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "laura.heath@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "emma.costa@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "ann.campton@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "jessica.lundin@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "tiara.adams@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "trisha.zintel@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "tera.derita@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "pranita.atri@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "jordan.driscoll@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "amelia.kallaher@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "melissa.klein@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "ram.ayyala@sagebase.org";
GRANT ROLE SAGE_AD_ANALYST TO USER "julia.gray@sagebase.org";

-- Restrict DATA_ACCESS_SUBMISSION_EVENT to SYNAPSE_EVENT_GOVERNANCE_READ only
-- SYNAPSE_DATA_WAREHOUSE
REVOKE SELECT, MONITOR
	ON DYNAMIC TABLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT.DATA_ACCESS_SUBMISSION_EVENT
	FROM DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_TABLE_READ;
GRANT SELECT, MONITOR
	ON DYNAMIC TABLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT.DATA_ACCESS_SUBMISSION_EVENT
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE.SYNAPSE_EVENT_GOVERNANCE_READ;
-- SYNAPSE_DATA_WAREHOUSE_DEV
REVOKE SELECT, MONITOR
	ON DYNAMIC TABLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT.DATA_ACCESS_SUBMISSION_EVENT
	FROM DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_TABLE_READ;
GRANT SELECT, MONITOR
	ON DYNAMIC TABLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT.DATA_ACCESS_SUBMISSION_EVENT
	TO DATABASE ROLE SYNAPSE_DATA_WAREHOUSE_DEV.SYNAPSE_EVENT_GOVERNANCE_READ;

-- Grant developers access to snowflake.account_usage schema
GRANT IMPORTED PRIVILEGES
ON DATABASE SNOWFLAKE
TO ROLE DATA_ENGINEER;
